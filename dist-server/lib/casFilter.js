 ...
 */
export function assertCasRuc(user, ruc, res) {
    if (!user.casId)
        return true;
    if (!user.casRUC) {
 ...
 */
export function applyCasIdFilter(req, user, _type) {
    if (!user.casId)
        return '';
 ...
 */
export function enforceCasRuc(user, requestedRuc) {
    if (user.casId)
        return user.casRUC ?? null;
